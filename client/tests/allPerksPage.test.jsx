...
describe('AllPerks page (Directory)', () => {
  ...
  */
  ...
  test('lists public perks and responds to merchant filtering', async () => {
    ...
    // Wait for baseline card to appear.
    await waitFor(() => {
      expect(screen.getByText(seededPerk.title)).toBeInTheDocument();
    });

    // Find the merchant select (combobox role) and choose the seeded merchant.
    const merchantSelect = screen.getByRole('combobox');
    fireEvent.change(merchantSelect, { target: { value: seededPerk.merchant } });

    // The page auto-searches on change; wait for the filtered result to appear.
    await waitFor(() => {
      expect(screen.getByText(seededPerk.title)).toBeInTheDocument();
    });
